...
function updateDate(){const date=dailyDate();$('date-display').textContent=new Date(date+'T12:00:00Z').toLocaleDateString('en-US',{month:'short',day:'numeric',timeZone:'UTC'}).toUpperCase();$('race-number').textContent=String(Math.max(1,Math.floor((Date.parse(date)-Date.UTC(2026,8,20))/86400000)+1)).padStart(3,'0')}
